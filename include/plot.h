...
void fit( const char * input ,const char * output, double mean , double sigma){
 ...
 //   double mean;
  ...
  /* EX funzione binning */
	...
	mean = 0.0;    
    for ( i = 0; i<n ; i++){
	  mean += freq[i]*(min+width*i);
	}
    mean /= (double) n_events;
    printf("mean %e\n",mean);
    
    sigma= 0.0;
    for ( i = 0; i<n ; i++){
	  sigma += (freq[i]*(min+width*i)-mean)*(freq[i]*(min+width*i)-mean);
	}
    sigma /= (double) n_events;
    printf("sigma:%e\n",sigma);
    */
    f=fopen("bin.dat","w");
    for(i=0;i<n;i++){
...
// 	//printf("A vale %lf \n ", n_events*width);
	fprintf(pipe, "fit f(x) 'bin.dat' via A,s,m\n");
