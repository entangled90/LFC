...
int main(){
	...
	*/
	...
	/* Allocazione della memoria*/
		...
		energy_mean_clustered = malloc((K_MAX-K_START)*sizeof(cluster_jk));
	/* Alloca i vettori per la struct cluster_jk e impone che il numero di configurazioni è N_BIN*/
		...
		init_cluster_jk(energy_mean_clustered, K_MAX -2 , N_BIN);
	/*
	 ...
	 * 	Il valor medio (per ogni tempo) viene salvato in deltaE_vector
	 */
	...
	*/
		vec_init(energy_mean_clustered->a, energy_mean_clustered->n_conf);
		for(i = 0; i< K_MAX - K_START; i++){
			clusterize((energy_clustered+i));
			for( j = 0; j< N_BIN; j++){
				energy_mean_clustered->a[j] += energy_clustered[i].a[j]/(double)3;
			}
		}
		for(i = 0; i< K_MAX - K_START; i++)
			deltaE_mean += deltaE_vector[i]/(double)(K_MAX-K_START);

		deltaE_variance = variance_cluster_jk(energy_mean_clustered, deltaE_mean);
	...
	/* Funzione che fa il free del vettore contenuto nella struct e poi della struttura */
		...
		free_cluster_jk(energy_mean_clustered , K_MAX - K_START);
