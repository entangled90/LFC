...
/*Cambiando questa funzione si cambia il tipo di sistema */
double f2 ( double x1, double x2, double time){
  //return ( 4*(1-x1*x1)*x2-x1); // Van der pol
  return (-sin(x1) -Q*x2+B*cos(OMEGA_EXT*time)); // Pendolo smorzato con forzante sinusoidale
}
...
void drawCircle(struct point2D p,double r)
{
	int i;
	rgb_t color;
	color = d2rgb(fabs(0.0)); //tail->x * tail->y));
...
void drawLine( struct point2D *head )
{
  int i = 0;
	...
	do{
		rgb_t color;
        color = d2rgb(0.0); //fabs( cursor->y*cursor->y));
        glColor3d(color.r,color.g,color.b);
