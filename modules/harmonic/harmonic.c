...
void matrix_element_cluster ( cluster_jk * input_E, cluster_jk * input_corr , cluster_jk *matrix_cluster){
	...
	for( k = 0 ; k < K_MAX -K_START ; k++){
		...
		matrix_cluster[k].a[i] =  input_corr[k+1].a[i]*exp(Nx/2.0*(input_E[k].a[i]))/(2.0*cosh((Nx/2.0-(k+K_START))*input_E[k].a[i]));
		matrix_cluster[k].a[i]= sqrt(matrix_cluster[k].a[i]);
		}
	matrix_cluster[k].mean  = input_corr[k+1].mean*exp(Nx/2.0*(input_E[k].mean))/(2*cosh((Nx/2.0-(k+K_START))*input_E[k].mean));
	matrix_cluster[k].mean= sqrt(matrix_cluster[k].mean);
	}
